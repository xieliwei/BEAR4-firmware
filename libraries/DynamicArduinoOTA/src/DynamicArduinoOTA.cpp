...
DynamicArduinoOTAClass::DynamicArduinoOTAClass()
: _port(0)
, _initialized(false)
...
DynamicArduinoOTAClass& DynamicArduinoOTAClass::onStart(THandlerFunction fn) {
    _start_callback = fn;
    return *this;
}

DynamicArduinoOTAClass& DynamicArduinoOTAClass::onEnd(THandlerFunction fn) {
    _end_callback = fn;
    return *this;
}

DynamicArduinoOTAClass& DynamicArduinoOTAClass::onProgress(THandlerFunction_Progress fn) {
    _progress_callback = fn;
    return *this;
}

DynamicArduinoOTAClass& DynamicArduinoOTAClass::onError(THandlerFunction_Error fn) {
    _error_callback = fn;
    return *this;
}

DynamicArduinoOTAClass& DynamicArduinoOTAClass::setPort(uint16_t port) {
...
DynamicArduinoOTAClass& DynamicArduinoOTAClass::setHostname(const char * hostname) {
...
String DynamicArduinoOTAClass::getHostname() {
    return _hostname;
}

DynamicArduinoOTAClass& DynamicArduinoOTAClass::setPassword(const char * password) {
...
DynamicArduinoOTAClass& DynamicArduinoOTAClass::setPasswordHash(const char * password) {
    if (!_initialized && !_password.length() && password) {
        _password = password;
    }
    return *this;
...
DynamicArduinoOTAClass& DynamicArduinoOTAClass::setPartitionLabel(const char * partition_label) {
...
String DynamicArduinoOTAClass::getPartitionLabel() {
    return _partition_label;
}

DynamicArduinoOTAClass& DynamicArduinoOTAClass::setRebootOnSuccess(bool reboot){
    _rebootOnSuccess = reboot;
    return *this;
}

DynamicArduinoOTAClass& DynamicArduinoOTAClass::setMdnsEnabled(bool enabled){
    _mdnsEnabled = enabled;
    return *this;
}

void DynamicArduinoOTAClass::begin() {
    if (_initialized){
...
int DynamicArduinoOTAClass::parseInt(){
...
String DynamicArduinoOTAClass::readStringUntil(char end){
    String res = "";
    int value;
    while(true){
        value = _udp_ota.read();
...
void DynamicArduinoOTAClass::_onRx(){
    if (_state == OTA_IDLE) {
        int cmd = parseInt();
...
void DynamicArduinoOTAClass::_runUpdate() {
...
void DynamicArduinoOTAClass::end() {
    _initialized = false;
    _udp_ota.stop();
    if(_mdnsEnabled){
        MDNS.end();
...
void DynamicArduinoOTAClass::handle() {
    if (!_initialized) {
        return;
    }
...
int DynamicArduinoOTAClass::getCommand() {
    return _cmd;
}

void DynamicArduinoOTAClass::setTimeout(int timeoutInMillis) {
    _ota_timeout = timeoutInMillis;
}

#if !defined(NO_GLOBAL_INSTANCES) && !defined(NO_GLOBAL_ARDUINOOTA)
DynamicArduinoOTAClass DynamicArduinoOTA;
#endif
